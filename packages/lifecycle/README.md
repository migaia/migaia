...
- [作用域与释放](#作用域与释放模块)：`createLifecycleScope`、`createSyncLifecycleScope`、`executeReleaseDescriptor`、`createDisposeTransaction`
...
**`validateSchedulerDelay` / `validateSchedulerTime`｜3 秒上手** —— 本包内部用来校验时间值的公开工具，自定义 scheduler 实现也可复用：
...
`validateSchedulerTime(value, label)`：`label: string` 必填，仅用于错误信息里标注字段名。`validateSchedulerDelay(value, label?)`：`label` 可选，默认 `'delayMs'`。两者返回值本身（校验通过时）。

**`createAbortController`｜5 秒上手** —— 本包内部使用的极简 `AbortController`，不依赖全局 `AbortController`：
...
无构造参数。`abort(reason?)` 幂等；对已中止的信号再 `addEventListener` 不触发监听器；某个监听器抛错时，其余监听器仍会跑完，单个失败包成 `ABORT_LISTENER_FAILED` 抛出，多个失败聚合成同码的 `AggregateError`。
...
**`LifecycleErrorCode`｜10 秒上手** —— 稳定错误码表（21 个码），唯一声明处，`source` 恒为 `'@migaia/lifecycle'`：
