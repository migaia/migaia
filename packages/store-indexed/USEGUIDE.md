...
本文是 `@migaia/store-indexed` 的完整参考手册。先看 [README.md](./README.md#5-最小示例) 的最小示例，跑起来之后再回来查这里的细节——README 讲"是什么、适合什么场景、五分钟怎么跑起来"，本文讲"每一个方法、每一个参数、每一种边界行为"。

## 目录

1. [设计动机与核心机制](#1-设计动机与核心机制)
2. [构造参考](#2-构造参考)
3. [`ObservableObject` 完整参考](#3-observableobject-完整参考)
4. [`ObservableArray` 完整参考](#4-observablearray-完整参考)
5. [`ObservableMap` 完整参考](#5-observablemap-完整参考)
6. [`ObservableSet` 完整参考](#6-observableset-完整参考)
7. [工厂函数参考](#7-工厂函数参考)
8. [错误信息全表](#8-错误信息全表)
9. [生命周期与资源释放](#9-生命周期与资源释放)
10. [与 `mutationGuard` 集成](#10-与-mutationguard-集成)
11. [多 Runtime 场景](#11-多-runtime-场景)
12. [常见问题排查](#12-常见问题排查)
...
每个集合内部维护两样东西：一份普通的 JS 存储（`Map`/数组/`Set`），和一个 `ReactiveCellPool`（键 → `Signal` 的池）。**只有第一次以"追踪方式"读某个 key/索引时，才会为它创建一个 `Signal`（cell）**：
...
- **从未被订阅也会回收**：如果一次 `getOrCreate` 之后，直到当前微任务结束都没有任何 effect/computed 订阅这个 cell（比如只是一次性读了一下），会在微任务尾部被当作"从未使用"回收，不会因为一次孤立的读而永久占用内存。
...
每个集合在构造时通过 `claimOwnership` 绑定到一个具体的 `IRuntime`。如果某处代码正处于**另一个** Runtime 的追踪状态中，却读了这个集合（属于不同的 Runtime），会立即抛错，而不是把两张依赖图悄悄接到一起——那样产生的 bug 极难定位。同一个 Runtime 内部的多次追踪读取不受影响。
...
```ts
new ObservableObject<T>(initial: T, runtime?: IRuntime, options?: IObservableCollectionOptions)
new ObservableArray<T>(initial?: Iterable<T>, runtime?: IRuntime, options?: IObservableCollectionOptions)
new ObservableMap<K, V>(initial?: ReadonlyMap<K, V> | Iterable<readonly [K, V]>, runtime?: IRuntime, options?: IObservableCollectionOptions)
new ObservableSet<T>(initial?: Iterable<T>, runtime?: IRuntime, options?: IObservableCollectionOptions)
```
...
| 方法 | 参数类型 | 返回值 | 追踪读/写 | 同步/异步 | 副作用 |
| --- | --- | --- | --- | --- | --- |
| `get(key)` | `key: K` | `T[K]` | 追踪读，惰性建 cell | 同步 | 无 |
...
| `replace(next)` | `next: T` | `void` | 写 | 同步 | 计算出多余的 key 先逐个 `delete`，再对 `next` 的每个 key 调 `set`；整体在一次 `batch()` 内完成 |
| `prune()` | 无 | 本次回收的 cell 数 | — | 同步 | 对每个已不在当前 key 集合里的 cell，尝试立即 tombstone（仅当它已无订阅者） |
| `dispose()` | 无 | `void` | — | 同步 | 释放全部拥有的 Signal；重复调用是安全的空操作 |

写操作前都会先调用 `assertMutation(operation)`（含 `assertActive()` 与 `mutationGuard?.assertMutationAllowed()`），已 dispose 的集合或未通过 guard 校验的写操作会在这里抛错。

---

## 4. `ObservableArray` 完整参考
...
| 方法 | 参数类型 | 返回值 | 追踪读/写 | 同步/异步 | 副作用 |
| --- | --- | --- | --- | --- | --- |
| `length` | — | `number` | 追踪读（依赖结构信号） | 同步 | 无 |
| `at(index)` | `index: number`（支持负数，从末尾数） | `T \| undefined` | 追踪读，惰性建 cell；越界或负索引都会额外依赖结构信号 | 同步 | 无。非整数索引抛 `TypeError` |
| `snapshot()` | 无 | 冻结数组 | 追踪读（依赖修订信号） | 同步 | 无 |
| `peek()` | 无 | 冻结数组 | 非追踪读 | 同步 | 无，不建 cell |
| `set(index, value)` | `index: number, value: T` | `void` | 写 | 同步 | 越界抛 `RangeError`；值与旧值 `Object.is` 相等时直接跳过（不触发任何信号）；否则更新已建的 cell 并触发修订信号 |
| `push(...values)` | `values: readonly T[]`（rest 参数） | 追加后的新长度 | 写 | 同步 | 空参数直接返回当前长度、不触发信号；否则为已建 cell 的新增位置同步值，触发结构信号与修订信号，整体在一次 `batch()` 内完成 |
| `pop()` | 无 | 被移除的最后一项，或空数组时 `undefined` | 写 | 同步 | 空数组不触发任何信号；否则把末位 cell 置为缺失，触发结构信号与修订信号 |
| `splice(start, deleteCount?, ...items)` | `start: number, deleteCount?: number, items: readonly T[]` | 被删除项组成的冻结数组 | 写 | 同步 | 内部构造出新数组后整体走 `replace` 逻辑 |
| `replace(values)` | `values: Iterable<T>` | `void` | 写 | 同步 | 见下方 `#replaceInternal` 说明 |
| `clear()` | 无 | `void` | 写 | 同步 | 等价于 `replace([])` |
| `prune()` | 无 | 本次回收的 cell 数 | — | 同步 | **对全部已物化 cell 无条件尝试 tombstone**（`isLive` 恒为 `false`），不只是越界的 cell——`splice`/`pop` 之后"索引"这个身份本身就不再稳定，任何一个既存 cell 都可能对应错的位置，因此凡是无订阅者的都值得回收 |
| `dispose()` | 无 | `void` | — | 同步 | 释放全部拥有的 Signal |
...
| 方法 | 参数类型 | 返回值 | 追踪读/写 | 同步/异步 | 副作用 |
| --- | --- | --- | --- | --- | --- |
| `size` | — | `number` | 追踪读（依赖结构信号） | 同步 | 无 |
| `get(key)` | `key: K` | `V \| undefined` | 追踪读，惰性建 cell | 同步 | 无 |
| `peek(key)` | `key: K` | `V \| undefined` | 非追踪读 | 同步 | 无，不建 cell |
| `has(key)` | `key: K` | `boolean` | 追踪读，惰性建 cell（membership 本身按 key 追踪，不是靠结构信号） | 同步 | 无 |
...
| `snapshot()` | 无 | 新建的普通 `Map` 副本 | 追踪读（依赖迭代信号） | 同步 | 无 |
| `prune()` | 无 | 本次回收的 cell 数 | — | 同步 | 对每个已不在当前 key 集合里的 cell 尝试 tombstone |
| `dispose()` | 无 | `void` | — | 同步 | 释放全部拥有的 Signal |
...
| --- | --- | --- | --- | --- | --- |
| `size` | — | `number` | 追踪读（依赖结构信号） | 同步 | 无 |
| `has(value)` | `value: T` | `boolean` | 追踪读，惰性建 cell（membership 按值追踪） | 同步 | 无 |
...
`ObservableSet` 没有单独的修订/迭代信号——它只有一个结构信号，因为"成员是谁"和"内容是否变化"对 Set 而言是同一件事。
...
**参数顺序是 `(initial, options, runtime)`**，而对应的类构造函数是 `(initial, runtime, options)`——两者故意/historically 不一致，混着用最容易踩坑：
...
一个集合从构造开始就通过 `claimOwnership` 绑定到指定（或默认）的 Runtime；使用期间每次追踪读都可能新建 cell（Signal），这些 cell 全部登记进集合自己的所有权集合；`dispose()` 时逐一调用它们的 `.dispose()` 断开下游依赖边，再清空登记表。**重复调用 `dispose()` 是安全的空操作**（内部有 `#disposed` 标志短路）。
...
具体如何进入"action 作用域"（比如 `MutationPolicy` 的 action 计数如何递增）属于 `@migaia/store-middleware` 自己的机制，参见该包文档；`store-indexed` 这边只负责在每次写操作前调用一次 `assertMutationAllowed(operation)`，`operation` 参数形如 `` `${debugName}.set(key)` ``，便于错误信息定位到具体是哪个集合的哪次调用。
...
**同一个集合实例只能属于它构造时绑定的那一个 Runtime**。如果在另一个 Runtime 的 effect/computed 求值过程中读了这个集合，会抛出跨 Runtime 错误（见 [第 8 节](#8-错误信息全表)）——这不是可配置行为，是刻意的保护，防止两张本应独立的依赖图被意外接在一起。
...
不一样。类构造函数第二个参数是 `runtime`，工厂函数第二个参数是 `options`。混用会把值传错位置，参见 [第 7 节](#7-工厂函数参考)。
...
`keys()`（以及 `ObservableMap.size`）只依赖结构信号，只关心 key 集合本身是否变化；`snapshot()` 依赖修订/迭代信号，任意值变化都会触发。这是刻意的追踪粒度拆分，见 [1.3 节](#13-结构信号--修订信号--迭代信号)。
