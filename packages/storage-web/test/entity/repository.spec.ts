...
import { registerIndexedDbBackfillStore } from '../../src/backends/indexed-db-backfill'
...
it('SWV2-T39 routes complete indexed entity put/remove through planner', async () => {
  ...
  const indexedStore = {
    ...base,
    capabilities: { ...base.capabilities, secondaryIndexes: true },
    ensureRecordIndexes: async (scope: string) => ({
      scope,
      generation: 'generation-1',
      fingerprint: 'fingerprint-1'
    }),
    getRecordIndexReadiness: async () => ({ status: 'complete' as const, scanned: 1, indexed: 1 }),
    putIndexedRecord: async () => 'unused',
    iterateRecordIndex: async function* () {},
    transactionIndexed: async (_handle: unknown, run: (tx: unknown) => Promise<unknown>) => {
      calls.push('transactionIndexed')
      return run({
        put: async (_value: unknown, _key: unknown, projection: unknown) => {
  ...
  const entity = defineEntity<IUser>()({
    name: 'planner-routed-users',
    key: 'id',
    indexes: { email: { path: 'email' } }
  })
  const repository = entity.connect(indexedStore)
  await repository.put({ id: 'u1', name: 'Ada', email: 'ada@example.com' })
  await repository.remove('u1')
  expect(calls).toEqual(['transactionIndexed', 'transactionIndexed'])
  expect(projections[0]).toEqual({ email: { kind: 'single', key: 'ada@example.com' } })
})

it('SWV2-T39 fallback batch does not execute secondary-index selectors', async () => {
  let selectorCalls = 0
  const entity = defineEntity<IUser>()({
    name: 'fallback-batch-users',
    key: 'id',
    indexes: {
      email: {
        select: () => {
          selectorCalls += 1
          throw new Error('fallback must not project')
        },
        revision: 1
      }
    }
...
it('SWV2-T39 routes complete indexed migration through planner projections', async () => {
  ...
  const indexedStore = {
    ...
    iterateRecordIndex: async function* () {},
    transactionIndexed: async (_handle: unknown, run: (tx: unknown) => Promise<unknown>) => {
      indexedTransactions.push(1)
      return base.transaction(async (tx) =>
        run({
          get: tx.get,
          put: async (value: unknown, key: unknown, projection: unknown) => {
            projections.push(projection)
            return tx.put(value, key as IStorageKey)
          },
          delete: tx.delete
        })
      )
  ...
  const v1 = defineEntity<IUser>({ name: 'planner-migrate-users', key: 'id' })
  await v1.connect(indexedStore).put({ id: 'u1', name: 'Ada', email: 'ada@example.com' })
  await base.putRecord(
    { __v: 1, data: { id: 'legacy', name: 'Legacy', email: 'legacy@example.com' } },
    ['planner-migrate-users', 'legacy']
  )
  const v2 = defineEntity<IUser>()({
    name: 'planner-migrate-users',
    key: 'id',
    version: 2,
    migrations: { 2: async (value) => value },
    indexes: { email: { path: 'email' } }
  })
  await expect(v2.connect(indexedStore).migrate()).resolves.toMatchObject({ migrated: 2 })
  expect(indexedTransactions).toHaveLength(1)
...
it('SWV2-T39 keeps indexed migration conflict retries projected and atomic', async () => {
  ...
  const indexedStore = {
    ...
    getRecordIndexReadiness: async () => ({ status: 'complete' as const, scanned: 2, indexed: 2 }),
    putIndexedRecord: async () => 'unused',
    iterateRecordIndex: async function* () {},
    transactionIndexed: async (_handle: unknown, run: (tx: unknown) => Promise<unknown>) => {
      indexedTransactions += 1
      if (indexedTransactions === 1)
  ...
  const v1 = defineEntity<IUser>({ name: 'planner-conflict-users', key: 'id' })
  await v1.connect(indexedStore).put({ id: 'u1', name: 'Ada', email: 'ada@example.com' })
  await base.putRecord(
    { __v: 1, data: { id: 'legacy', name: 'Legacy', email: 'legacy@example.com' } },
    ['planner-conflict-users', 'legacy']
  )
  const v2 = defineEntity<IUser>()({
    name: 'planner-conflict-users',
    key: 'id',
    version: 2,
    migrations: { 2: async (value) => value },
    indexes: { email: { path: 'email' } }
  ...
  await expect(v2.connect(indexedStore).migrate()).resolves.toMatchObject({ migrated: 2 })
  expect(indexedTransactions).toBe(3)
  expect(projections).toHaveLength(2)
  expect(projections).toEqual([
    { email: { kind: 'single', key: 'ada@example.com' } },
    { email: { kind: 'single', key: 'legacy@example.com' } }
  ])
})
...
it('SWV2-T34 exposes projection and failed-readiness persistence errors in stable order', async () => {
  ...
  registerIndexedDbBackfillStore(store, {
    ensureRecordIndexes: async (scope) => ({ scope, generation: 'g1', fingerprint: 'f1' }),
    getRecordIndexReadiness: async () => ({ status: 'running', scanned: 0, indexed: 0 }),
    openBackfillSession: async () => ({
      generation: 'g1',
      ownerToken: 'owner-1',
      renew: async () => {},
      readBatch: async (_ctx, options) => {
        const candidate = {
          key: ['aggregate-backfill-users', 'u1'] as IStorageKey,
      ...
      commitBatch: async () => ({ status: 'running', scanned: 0, indexed: 0 }),
      fail: async (cause) => {
        reportedPrimary = cause
  ...
  const failure = await repository.findManyBy('email').catch((cause: unknown) => cause)
  expect(failure).toBeInstanceOf(AggregateError)
  expect((failure as AggregateError).cause).toBe(projectionFailure)
  expect((failure as AggregateError).errors).toEqual([projectionFailure, persistenceFailure])
...
it('SWV2-T16 find APIs share fallback range, ordering, direction, and limit semantics', async () => {
  ...
  await expect(repository.findBy('email', 'same@example.com')).resolves.toMatchObject({ id: 'u1' })
  await expect(
    ...
    { id: 'u3', name: 'Cat', email: 'z@example.com' },
    { id: 'u2', name: 'Bob', email: 'same@example.com' }
  ])
  const streamed: IUser[] = []
  for await (const value of repository.streamBy('email', {
    lower: 'same@example.com',
    upper: 'same@example.com'
  }))
    streamed.push(value)
