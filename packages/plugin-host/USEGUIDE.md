...
| `host.config.get(path)` | `plugin.key` 或 `plugin.[index].key` | 必填 | `unknown \| undefined` | 同步 | 只能读取嵌套配置值；直接传插件名是非法的（详见 [§5](#5-配置系统)）。返回目标路径值；未知插件或缺失路径返回 `undefined`。对象/数组目标只做一层浅拷贝，嵌套值保持原引用。 |
| `host.config.update(name, recipe)` | `name: string`；`recipe(previous) => patch`，同步 plain record | 两项都必填 | `Promise<void>` | 异步 | 浅合并 patch，执行 `plugin.update(next, core)` 后提交；失败不提交；提交配置与 patch 完全隔离。 |
...
`getShared()` 返回 provider 明确共享的原始引用，不提供 config 式的浅拷贝隔离；provider 与 consumer 应共同约定其可变性和生命周期。
...
| `update(next, core)` | `next: Readonly<TConfig>`，候选完整配置 | 可选 | `void \| Promise<void>` | 视情况 | 响应 `config.update`；成功才提交 `next`。 |
...
| API / 签名 | 参数 | 必填性 | 返回值 | 同步/异步 | 作用 |
| --- | --- | --- | --- | --- | --- |
| `core.config.get<T>()` | 可选泛型 `T`，通常由插件 `config` 推导 | 无运行时参数 | `Readonly<T>` | 同步 | 当前插件的已提交配置浅快照；嵌套对象/数组按引用共享，由插件自行负责不可变性。 |
...
`recipe` 必须**同步**返回 plain record。Host 对 patch 做浅合并；嵌套对象应视为不可变值处理。`update()` 失败时旧配置保持不变。插件在 `update(next)` 里通过参数 `next` 读取候选配置，而 `core.config.get()` 返回的始终是已提交的配置，两者在 `update` 执行期间可能不同。
...
这是一个**明确的设计取舍**，不是遗漏。深拷贝在插件配置这种高频访问路径上的性能开销被认为不划算，配置读写 API 从一开始就承诺"顶层隔离、嵌套值保留引用身份"——需要真正不可变的嵌套配置，插件作者应该在 `config` 里存不可变数据结构（如 `Object.freeze` 过的对象），而不是依赖 Host 帮你深拷贝。读路径对非 plain-object 的嵌套值（`Date`、类实例、`Map` 等）直接返回引用而不校验其"纯净形状"，这个宽松度和写路径保持一致；浅拷贝校验只用于挡住 `__proto__`/原型链污染这类真实攻击面。
...
**Q：`config.get()` 读出来的嵌套对象被意外修改了。**
配置系统只做一层浅拷贝，嵌套值是共享引用，这是设计如此（见 [§5](#5-配置系统)），不要直接修改读出来的嵌套对象——如果需要修改，先自己 clone 一份。
