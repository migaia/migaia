...
import { WebRpcOutboundPipeline } from '../../src/internal/pipeline.js';
...
describe('outbound pipeline encoded type boundary', () => {
  ...
  it('prepares every authenticated chunk before sending and releases once on failure', async () => {
    ...
    const secondAuthenticationError = new WebRpcAuthenticationError(
  ...
  it('preserves a class transport receiver for ordinary and chunked sends', async () => {
    ...
    const chunkedPipeline = new WebRpcOutboundPipeline(
      transport,
      'a',
      protocol,
      {
