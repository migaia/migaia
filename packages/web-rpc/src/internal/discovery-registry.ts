...
export class DiscoveryRegistry {
  ...
  canRevokeCandidate(key: string, maxEntries: number): boolean {
    return this.#manualRevokedCandidates.has(key) || this.#manualRevokedCandidates.size < maxEntries
  }
  ...
  close(reason: unknown): void {
    const errors = this.closeAndCollect(reason)
    if (errors.length > 0) {
  ...
  /** Collects every registry cleanup failure while preserving first-failure order. */
  closeAndCollect(reason: unknown): readonly unknown[] {
